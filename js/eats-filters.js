// function initializeFilters(data, eventBus) {
...
function initializeFilters(eats, eventBus) {
  ...
  const eatsCheckboxes = document.querySelectorAll('[name="place-filter-type"]');
  const barCheckboxes = document.querySelectorAll('[name="place-filter-type"]');
  const shoppingCheckboxes = document.querySelectorAll('[name="place-filter-type"]');
  const attractionCheckboxes = document.querySelectorAll('[name="place-filter-type"]');
  const eventCheckboxes = document.querySelectorAll('[name="place-filter-type"]');
...
//   // Create Attraction Checkbox Filter 
  ...
  function shouldShowEats(restaurant) {
   ...
   // let inSelectedEvent = false;
    ...
    // console.log(checkedEatsCBs.length);
   // console.log(restaurant.properties);
    if (checkedEatsCBs.length > 0) {
      for (const cb of checkedEatsCBs) {
        console.log(cb.value, ' ', restaurant.properties.type);
    ...
    console.log(inSelectedEats, inSelectedBar, inSelectedShopping)

    return inSelectedEats && inSelectedBar && inSelectedShopping 
    //&& inSelectedAttraction && inSelectedEvent
  }
